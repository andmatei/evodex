...
def test_invalid_joint_angle_limit_raises_error():
    ...
    with pytest.raises(ValidationError, match="joint_angle_limit.min must be less than joint_angle_limit.max"):
...
def test_out_of_bounds_joint_angle_limit_raises_error():
    ...
    with pytest.raises(ValidationError, match="joint_angle_limit must be between -pi and pi"):
...
def test_negative_segment_length_raises_error():
    ...
    with pytest.raises(ValidationError, match="Input should be greater than or equal to 0"):
        SegmentConfig(length=-0.1, width=0.05, mass=1, motor_stiffness=1, motor_damping=1, joint_angle_limit=(0,1))
